...
class MMIL_Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.visual_encoder = nn.TransformerEncoder \
            (nn.TransformerEncoderLayer(d_model=512, nhead=1, dim_feedforward=512), num_layers=1)
        self.joint_encoder = ta.models.Conformer(input_dim=512, num_heads=1, ffn_dim=512, num_layers=1,depthwise_conv_kernel_size=31)
    ...
    def forward(self, audio, visual, visual_st):
        ...
        out_=self.joint_encoder(torch.cat((x1,x2),axis=1),20*torch.ones((x1.shape[0],)).long().to("cuda:0"))[0]
        x1=out_[:,:10,:]
        x2=out_[:,10:,:]

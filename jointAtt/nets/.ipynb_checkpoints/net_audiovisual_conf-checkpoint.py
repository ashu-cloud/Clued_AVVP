...
class Encoder(nn.Module):
    ...
    def forward(self, src_a, src_v, mask=None, src_key_padding_mask=None):
        ...
        for i in range(self.num_layers):
            joint=torch.cat((src_a,src_v),axis=1)
            output_a = self.layers[i](src_a, joint, src_mask=mask,
                                    src_key_padding_mask=src_key_padding_mask)
            output_v = self.layers[i](src_v, joint, src_mask=mask,
                                      src_key_padding_mask=src_key_padding_mask)
...
class HANLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=512, dropout=0.1):
        ...
        self.pos=PositionalEncoding2(d_model,0.5)
...
class MMIL_Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.visual_encoder = nn.TransformerEncoder \
            (nn.TransformerEncoderLayer(d_model=512, nhead=1, dim_feedforward=512), num_layers=1)
        #self.audio_encoder2 = ta.models.Conformer(input_dim=512, num_heads=1, ffn_dim=512, num_layers=1,depthwise_conv_kernel_size=31)
        #self.visual_encoder2 = ta.models.Conformer(input_dim=512, num_heads=1, ffn_dim=512, num_layers=1,depthwise_conv_kernel_size=31)
    ...
    def forward(self, audio, visual, visual_st):
        ...
        sims = 1
        #xx1,xx2=self.audio_encoder2(x1,10*torch.ones((x1.shape[0],)).long().to("cuda:0"))[0],\
        #                            self.visual_encoder2(x2,10*torch.ones((x2.shape[0],)).long().to("cuda:0"))[0]
        
        xx1 = F.normalize(x1, p=2, dim=-1)        
        xx2 = F.normalize(x2, p=2, dim=-1)
        
        sims = xx1.bmm(xx2.permute(0, 2, 1)).squeeze(1) / 0.2
        ...
        a_att_norm=torch.linalg.vector_norm(av_att[:,:,0,:])
        v_att_norm=torch.linalg.vector_norm(av_att[:,:,1,:])
        return global_prob, a_prob, v_prob, frame_prob, sims, mask, (a_att_norm,v_att_norm)

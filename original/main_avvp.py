...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        #print(data_idx)
        optimizer.zero_grad()
        output, a_prob, v_prob, _, sims, mask, (a_norm, v_norm) = model(audio, video, video_st)
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.3f}\tLoss: {:.3f}\tLoss: {:.3f}\tLoss: {:.3f}'.format(
                epoch, batch_idx * len(audio), len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), loss1.item(),  loss2.item(), loss3.item(), loss4.item()))
            print("Norms for audio and visual are - ", a_norm, v_norm)
...
def eval(model, val_loader, set):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            output, a_prob, v_prob, frame_prob, a, is_real,_ = model(audio, video, video_st)
...
def main():
    ...
    if args.mode == 'train':
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=12, pin_memory = True)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            torch.save(model.state_dict(), args.model_save_dir + args.checkpoint +"_"+str(epoch)+".pt")

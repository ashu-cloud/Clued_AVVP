...
class MMIL_Net(nn.Module):
    ...
    def forward(self, audio, visual, visual_st, audio2):

        b, t, d = visual_st.size()
        x1 = self.fc_a(audio)
        x_fake = self.fc_a(audio2)
        x_audio = x1
        ...
        xx3 = F.normalize(x_audio, p=2, dim=-1)
        xx4 = F.normalize(x2, p=2, dim=-1)

        sims = xx2.bmm(xx1.permute(0, 2, 1)).squeeze(1) * 5
        sims2 = xx3.bmm(xx4.permute(0, 2, 1)).squeeze(1) * 5
        sims = sims.reshape(-1, 10)
        sims2 = sims2.reshape(-1, 10)
        ...
        return global_prob, a_prob, v_prob, frame_prob, sims, mask

...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        data_idx = sample['idx']
        optimizer.zero_grad()
        output, a_prob, v_prob, _, sims, mask,(a_norm, v_norm) = model(audio, video, video_st, audio2)
...
def eval(model, val_loader, set):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            output, a_prob, v_prob, frame_prob, a, is_real,_ = model(audio, video, video_st, audio)
